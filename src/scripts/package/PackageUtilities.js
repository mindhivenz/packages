...
export default class PackageUtilities {
  static getPackages() {
    ...
    glob
      .sync(path.join(config.sourcePath, '*/'), globOpts)
      .forEach(globResult => {
  ...
  static filterPackages(packagesToFilter, { scope, ignore }, negate = true) {
    ...
    if (scope) {
      ...
      if (!packages.length) {
        throw new Errors.PackageNotFoundError(
          `No packages found that match '${scope}'`
        )
      }
    }
    ...
    if (ignore) {
      packages = packages.filter(pkg =>
        filterPackage(pkg.npmName, ignore, negate)
      )

      if (!packages.length) {
        throw new Errors.NoPackagesLeftError(
          `No packages remain after ignoring '${ignore}'`
        )
      }
    }
